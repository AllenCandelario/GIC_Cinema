...
namespace GIC_Cinema.Tests
{
    public class SeatAllocatorTests
    {
        ...
        public void DefaultAllocation_CenterOut_FromFurthestRow()
        {
            ...
            Assert.Equal(new Seat(0, 2), seats[0]);
            Assert.Equal(new Seat(0, 3), seats[1]);
            Assert.Equal(new Seat(0, 1), seats[2]);
            Assert.Equal(new Seat(0, 4), seats[3]);
        }
        ...
        public void AllocateFromPosition_FillRight_ThenNextRowsCenterOut()
        {
            ...
            movie.ConfirmBooking(new Booking("GIC0001", new() { new Seat(0, 2), new Seat(0, 3) }));

            var result = SeatAllocator.AllocateFromPosition(movie, seatNumber: 5, startingPosition: new Seat(0, 1));

            Assert.Equal(5, result.Count);
            Assert.Equal(new Seat(0, 1), result[0]);
            Assert.Equal(new Seat(0, 4), result[1]);
            Assert.Equal(new Seat(1, 2), result[2]);
            Assert.Equal(new Seat(1, 3), result[3]);
            Assert.Equal(new Seat(1, 1), result[4]);
        }
        ...
        public void Allocation_OverflowsAcrossRows_WhenRowNotEnough()
        {
            ...
            Assert.Contains(new Seat(0, 1), seats);
            Assert.Contains(new Seat(0, 2), seats);
            Assert.Contains(new Seat(0, 0), seats);
            Assert.Contains(new Seat(1, 1), seats);
            Assert.Contains(new Seat(1, 2), seats);
        }
        ...
        public void Allocation_SkipsOccupiedSeats()
        {
            ...
            Assert.DoesNotContain(new Seat(0, 2), seats);
            Assert.DoesNotContain(new Seat(0, 3), seats);
        }
    }
}
